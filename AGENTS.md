...
- Keep inputs approachable: curated asset lists, drag‑friendly allocation editing, and sensible defaults for equities, ETFs, and crypto.
...
- **Planned**: Create `InteractivePieChart` with custom pointer handlers (Chart.js alone—`chartjs-plugin-dragdata` is incompatible with the pie resizing UX we want).
- **Planned**: Add `ProjectionView` slider (1–10 years) whose result text turns red beyond 5 years and always shows a “longer-term uncertainty” disclaimer.
...
- **Next Up**:
  ...
  2. Update Monte Carlo simulations to include contributions, detect asset types, and emit both dollar and percentage projections (P10/P50/P90 + mean for 1–10 years).
...
## Backlog (Prioritized)
1. Contribution-aware Monte Carlo output (percentages + dollar projections, 1–10 years).
...
Items 1–4 unblock the interactive projection experience; downstream tasks (5–6) depend on the richer data model.
