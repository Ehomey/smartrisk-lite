...
function ProjectionSlider({ projections, initialInvestment = 10000 }) {
  ...
  if (!projections || !projections.percentiles) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Portfolio Projections</h3>
  ...
  const returnColorClass = selectedYear > 5 ? 'text-red-600' : 'text-green-600';
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-lg font-semibold text-gray-900">{selectedYear} Year{selectedYear > 1 ? 's' : ''}</span>
        </div>
      ...
      <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="text-center">
          <p className="text-sm text-gray-600 mb-1">Projected Median Return</p>
          <p className={`text-3xl font-bold ${returnColorClass}`}>
            {p50Return >= 0 ? '+' : ''}{p50Return.toFixed(1)}%
          </p>
          <p className="text-xs text-gray-500 mt-1">over {selectedYear} year{selectedYear > 1 ? 's' : ''}</p>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div>
              <p className="text-gray-500 mb-1">Pessimistic (P10)</p>
              <p className="font-semibold text-gray-700">{p10Return >= 0 ? '+' : ''}{p10Return.toFixed(1)}%</p>
            </div>
            <div>
              <p className="text-gray-500 mb-1">Median (P50)</p>
              <p className="font-semibold text-gray-700">{p50Return >= 0 ? '+' : ''}{p50Return.toFixed(1)}%</p>
            </div>
            <div>
              <p className="text-gray-500 mb-1">Optimistic (P90)</p>
              <p className="font-semibold text-gray-700">{p90Return >= 0 ? '+' : ''}{p90Return.toFixed(1)}%</p>
            </div>
          </div>
        </div>
      </div>
