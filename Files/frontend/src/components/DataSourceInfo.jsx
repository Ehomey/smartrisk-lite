...
function DataSourceInfo({ dataSources }) {
  ...
  const getBadgeColor = (source) => {
    if (source === 'cache') return 'bg-green-100 text-green-800 border-green-300';
    if (source === 'yfinance') return 'bg-blue-100 text-blue-800 border-blue-300';
    if (source === 'alpha_vantage') return 'bg-purple-100 text-purple-800 border-purple-300';
    if (source.includes('fallback')) return 'bg-amber-100 text-amber-800 border-amber-300';
          ...
          <div key={source} className="flex items-start">
            ...
            <div className="ml-3 flex-1">
              <div className="flex flex-wrap gap-2">
                {tickers.map((ticker) => (
                  <span key={ticker} className="inline-flex items-center px-2 py-1 rounded bg-gray-100 text-gray-700 text-xs font-medium">
                    {ticker}
                  </span>
                ))}
              </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <p className="text-xs text-gray-500">
          ...
          {Object.keys(groupedSources).some(s => s.includes('fallback')) && (
            <span className="text-amber-600"> Fallback occurred due to rate limits or errors with the primary source.</span>
          )}
        </p>
      </div>
    </div>
